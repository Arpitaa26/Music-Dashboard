...
export class PerformanceReportComponent implements OnInit {
  ...
  getModuleByLevel(id: string, index: number) {
    ...
    this.userData.callApi(
        ...
        var uniquelist:any = [];
        this.c_name.data.forEach(function (data:any) {
          if (!uniquelist[data.course_id]) {
            uniquelist[data.course_id]=1;
            console.log(uniquelist)
        
          classObj.courseListInterface_uniq.push(data);
          }
             
        });
       console.log( classObj.courseListInterface_uniq);
      });
  }
  report:any=[]

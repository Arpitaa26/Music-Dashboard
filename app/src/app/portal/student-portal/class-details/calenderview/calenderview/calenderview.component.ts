...
export class CalenderviewComponent implements OnInit, AfterViewInit {
  calendarVal: any;
  ...
  ngOnInit(): void {
    this.userData
      ...
      .subscribe((result: ICalendarResponse) => {
        ...
        this.events = this.calendarVal.map((book: any) => {
          ...
          return {
            ...
            title: '<div>'+formatDate(
              new Date(book.start_time),
              'h:mm a',
              'en-US'
            )+'</div><div>' + book.course_name, //+ '</div><div>'+book.batch_code+'</div>',
            color:this.color,
  ...
  redirectToSupport(item: ICalendarItem) {
    ...
    this.router.navigate([
      '/portal/students/support/task-create/',
      item.id,
      item.module_name,
      item.start_time,
    ]);
...
import { CalendarStatusEnum, ICalendarItem, ICalendarResponse } from 'src/app/portal/teacher-portal/class-details/calendarview/interfaces/calendar-item.interface';

@Pipe({
  name: 'calendarStatusStudent'
})
export class CalendarStatusStudentPipe implements PipeTransform {

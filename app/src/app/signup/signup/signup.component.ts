...
import { user_pronoun } from 'src/app/signup/user_pronoun_get_all';
import { user_type } from 'src/app/basic-module/reschedule/interface/user_type';
...
export class SignupComponent implements OnInit {
  ...
  user_pronoun: undefined | null | user_pronoun;
  user_type_id: undefined | null | user_type;
  emailRegex = /^[\w-\.]+@([\w-]+\.)+[\w-]{2,5}$/g;
  integerRegex = /^\d+$/;
  ...
  ngOnInit(): void {
    ...
    // let r_id:any = this.userRoute.snapshot.params['refid'];
    // this.r_id = this.activeRout.snapshot.paramMap.get('refid');
    ...
    this.form = this.fb.group(
      {
        ...
        // user_pronoun_id:['', [Validators.required]],
        user_pronoun_id: ['', [Validators.required]],
        phone_no: [
          '',
          [
        ...
        last_name: [''],
        user_type_id: [''],
        agreeCheckbox: [false, Validators.required],
      },
      {
  ...
  redirecttoterms(): void {
    window.location.href = 'https://thesvpacademy.com/terms-of-services/';
  }

  //-------previuously written fomr declaration-----------
  // form = new FormGroup({
  ...
  //   re_type: new FormControl('', [Validators.required]),
  //   user_pronoun_id: new FormControl('', [Validators.required]),
  ...
  //   agreeCheckbox: new FormControl(false, Validators.required)
  // });
  ///-------------------------------End of declaration------------------------------
  ...
  onRegister(): void {
    ...
    let nameAll = this.form.controls['name'].value?.split(' ');
    if (nameAll) {
      this.form.controls['first_name'].setValue(nameAll[0]);
      this.form.controls['last_name'].setValue(nameAll[1]);
    }
    // this.form.controls['user_pronoun_id'].setValue('1');
    this.form.controls['user_type_id'].setValue('5');
    if (
      this.form.controls['password'].value ===
      this.form.controls['re_type'].value
    ) {
      let inpData = this.form.value;

      if (inpData.hasOwnProperty('agreeCheckbox')) {
        delete inpData.agreeCheckbox;
      }
      console.log(inpData);
      this.activeRout.queryParams.subscribe((params) => {
        let refid: any = params['refid'];

        // const id:Number = this.activeRout.snapshot.params['refid'];
        //console.log(id);
        const updatedInpData = { ...inpData, refid };

        console.log('Form values with id  ', updatedInpData);
...
function passwordMatchValidator(): ValidatorFn {
  return (formGroup: AbstractControl): ValidationErrors | null => {
    ...
    const retypePassword = formGroup.get('re_type')?.value;

    if (password !== retypePassword) {
      formGroup.get('re_type')?.setErrors({ passwordMismatch: true });
    } else {
      formGroup.get('re_type')?.setErrors(null);
    }

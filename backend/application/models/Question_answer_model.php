...
class Question_answer_model extends CI_Model
{
  ...
  public function get_all($user_id = null,  $limit = null, $offset = 0)
  {

    $this->db->select("qa.*, concat(u.first_name,' ',u.middle_name,' ',u.last_name) AS user_fullname");
    $this->db->from("{$this->table} qa");
    $this->db->join("{$this->table_users} u", "u.id =qa.user_id");
